...
from contract_caller import init_context, copy_context, brief_context
...
while amount_a1 <= max_attack1:
    ...
    while amount_a2 <= max_attack2:
        contexts = {"pool1": brief_context(context0),
                    "pair": brief_context(context2)}
        ...
        pool_context_step1 =  brief_context(contexts["pool1"])
        pair_context_step1 =  brief_context(contexts["pair"])
        params = []
        with Pool(processes=os.cpu_count()) as p:
            for ts1, ts2 in tick_shifts:
                contexts = {"pool1": brief_context(pool_context_step1),
                            "pair": pair_context_step1}
                inputs = {"ETH_amount1": amount_a1,
                          "amount1": int(amount_a1 * MULTIPLIER),
                          "ETH_amount2": amount_a2,
                          "amount2": int(amount_a2 * MULTIPLIER),
                ...
                params.append((inputs, targets, contexts, 2))
            result = p.starmap(simulated_attack2, params, chunksize=100) #(inputs, targets, contexts, step=2)
